...
description: How to preserve TakeDeep threads and settings in dev containers
---

# Dev Container Persistence

When using TakeDeep in development containers (VS Code Dev Containers, GitHub Codespaces, etc.), your threads and settings can persist across container rebuilds by properly configuring volume mounts.

## Why Persistence Matters

Dev containers are ephemeral by default - when you rebuild the container, all data is lost unless explicitly persisted. TakeDeep stores important data including:

- **Conversation threads**: Your ongoing discussions with TakeDeep
- **Settings**: API configurations, custom modes, and preferences
- **Cache**: Vector store for code indexing and browser tool data

## Required Configuration

The TakeDeep dev container is pre-configured with named volumes to preserve your data. If you're setting up your own dev container, add these mounts to your `devcontainer.json`:
  ...
  "mounts": [
    {
      ...
      "target": "/root/.vscode-remote/data/User/globalStorage/takedeep.takedeep",
      "type": "volume"
    },
    {
...
|---|---|
| Threads | `/root/.vscode-remote/data/User/globalStorage/takedeep.takedeep/tasks/` |
| Settings | `/root/.vscode-remote/data/User/settings/` |
| Cache | `/root/.vscode-remote/data/User/globalStorage/takedeep.takedeep/cache/` |
| Vector Store | `/root/.vscode-remote/data/User/globalStorage/takedeep.takedeep/vector/` |
...
2. Start new conversations with TakeDeep
...
docker cp <container-name>:/root/.vscode-remote/data/User/globalStorage/takedeep.takedeep ./kilocode-backup
```
...
2. Set the custom storage path in VS Code settings:
   ...
   - Search for "TakeDeep: Custom Storage Path"
...
GitHub Codespaces automatically persists your VS Code settings and extensions. For TakeDeep threads, the pre-configured dev container includes the necessary volume mounts.
