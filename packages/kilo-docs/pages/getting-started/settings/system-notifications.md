...
description: "Configure native OS notifications for TakeDeep"
platform: legacy
---
...
TakeDeep uses system notifications to inform you about:
...
TakeDeep's system notifications work on all major operating systems with different underlying technologies:
...
No additional setup required. TakeDeep uses macOS's built-in command to display notifications.
...
**How it works:** TakeDeep first attempts to use `terminal-notifier` and automatically falls back to AppleScript if it's not installed.
