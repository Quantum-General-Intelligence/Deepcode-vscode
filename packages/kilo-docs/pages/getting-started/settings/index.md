---
title: "Settings"
description: "Configure TakeDeep settings and preferences"
---

# Settings

The VS Code extension can be configured through the Settings window, opened by pressing the gear icon in TakeDeep. Changes apply across extension surfaces, including the sidebar and Agent Manager. The CLI can also use the same JSONC config files when you use it directly.

## Configuring with the Agent

The fastest way to change your Kilo configuration is to ask the agent to do it for you. The agent has a built-in skill that understands the full `takedeep.jsonc` schema and can read, create, and update your config files directly.
...
The VS Code extension provides a **Settings webview UI** accessible from TakeDeep by clicking the gear icon ({% codicon name="gear" /%}). The UI is organized into tabs including Providers, Auto-Approve, Models, and more.
...
- **Global config:** `~/.config/kilo/takedeep.jsonc` — applies to all projects. On Windows, this is `C:\Users\<username>\.config\kilo\takedeep.jsonc`.
- **Project config:** `takedeep.jsonc` in your project root, or `.takedeep/takedeep.jsonc` for a cleaner setup. The `.takedeep/` version takes priority if both exist.
...
You can export and import settings from the **About TakeDeep** tab in the Settings UI:
...
Config files are also plain-text and portable — you can copy `~/.config/kilo/takedeep.jsonc` between machines directly.
...
{% callout type="warning" %}
If you check `takedeep.jsonc` into version control, make sure it does not contain API keys or other secrets (e.g., `provider.*.options.apiKey`). Use environment variables for credentials instead.
{% /callout %}
...
- **Global config:** `~/.config/kilo/takedeep.jsonc` -- applies to all projects. On Windows, this is `C:\Users\<username>\.config\kilo\takedeep.jsonc`.
- **Project config:** `takedeep.jsonc` in the root of your project -- overrides global settings for that project.
...
3. **Global** -- `~/.config/kilo/takedeep.jsonc`
4. **Custom** -- additional custom config paths
5. **Project** -- `takedeep.jsonc` in the project root
6. **`.kilo` directory** -- config from a `.takedeep/` directory in the project
...
When you create or open a `takedeep.jsonc` file, the CLI automatically injects a `$schema` property pointing to the config JSON schema. This gives you **autocompletion and validation** in any editor that supports JSON Schema (VS Code, JetBrains, etc.).

### Export and Import

There is no traditional export/import of settings -- the JSONC config files themselves are portable. Copy `~/.config/kilo/takedeep.jsonc` or `takedeep.jsonc` to another machine and you're done.
...
TakeDeep allows you to manage your configuration settings effectively through export, import, and reset options. These features are useful for backing up your setup, sharing configurations with others, or restoring default settings if needed.

You can find these options at the bottom of the TakeDeep settings page, accessible via the gear icon ({% codicon name="gear" /%}) in the TakeDeep chat view.

{% image src="/docs/img/settings-management/settings-management.png" alt="Export, Import, and Reset buttons in TakeDeep settings" width="800" caption="Export, Import, and Reset buttons" /%}

### Export Settings

Clicking the **Export** button saves your current TakeDeep settings to a JSON file.
...
- **Process:**
  ...
  3.  TakeDeep reads the file, validates its contents against the expected schema, and applies the settings.
...
Clicking the **Reset** button completely clears all TakeDeep configuration data and returns the extension to its default state. This is a destructive action intended for troubleshooting or starting fresh.
...
- **What is Reset:**
  ...
  - **Secret Storage:** All API keys and other secrets managed by TakeDeep are cleared.
  - **Task History:** The current task stack is cleared.

- **Result:** TakeDeep returns to its initial state, as if freshly installed, with default settings and no user configurations.

Use this option only if you are certain you want to remove all TakeDeep data or if instructed during troubleshooting. Consider exporting your settings first if you might want to restore them later.
...
Advanced options not exposed in the UI can be configured via the `experimental` key in `takedeep.jsonc`:
...
Refer to the auto-generated `$schema` in your `takedeep.jsonc` for the full list of available options.

{% /tab %}
{% tab label="CLI" %}

The CLI does not currently expose the same experimental feature toggles as the **VSCode (Legacy)** version. Configuration of model behavior, file editing strategies, telemetry, and other advanced options is handled directly in the JSONC config files. Refer to the auto-generated `$schema` in your `takedeep.jsonc` for the full list of available options.

Telemetry is enabled by default. Set `experimental.openTelemetry` to `false` in `takedeep.jsonc` to opt out. If `OTEL_EXPORTER_OTLP_ENDPOINT` is set in the environment, the CLI also exports OpenTelemetry traces and logs to that OTLP HTTP endpoint.

{% /tab %}
{% tab label="VSCode (Legacy)" %}

{% callout type="info" %}
These features are experimental and may change in future releases. They provide advanced control over TakeDeep's behavior for specific use cases.
{% /callout %}

### Concurrent File Edits

When enabled, TakeDeep can edit multiple files in a single request. When disabled, TakeDeep must edit one file at a time.
...
When enabled, TakeDeep will remind the model about the details of its current mode definition more frequently. This leads to stronger adherence to role definitions and custom instructions, but will use more tokens per message.
...
You can find this setting in the TakeDeep settings under 'Advanced Settings'.
