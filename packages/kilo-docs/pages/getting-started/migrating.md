...
description: "Guide for migrating to TakeDeep from other AI coding tools"
---

# Migrating from Cursor or Windsurf

Quickly migrate your custom rules from Cursor or Windsurf to TakeDeep. The process typically takes just a few minutes per project.

{% callout type="info" title="Two Workflow Approaches"%}

TakeDeep supports **two complementary workflows**—choose the one that fits your style, or use both:
...
## Why TakeDeep's Rules System?

TakeDeep simplifies AI configuration while adding powerful new capabilities:
...
### What's Different in TakeDeep

| Cursor | TakeDeep | Key Difference |
|---|---|---|
...
Cursor's `globs` field specifies which files a rule applies to. TakeDeep handles this through **mode-specific directories** instead.
...
**TakeDeep approach (Option 1 - Mode-specific directory):**
...
**TakeDeep approach (Option 2 - Custom mode):**
...
Cursor supports nested `.cursor/rules/` directories. TakeDeep uses flat structure with descriptive names:

```bash
# Cursor: .cursor/rules/backend/server/api-rules.mdc
# TakeDeep: .kilocode/rules/backend-server-api-rules.md
```
...
| Windsurf | TakeDeep | Key Difference |
|---|---|---|
...
Windsurf configures activation through the GUI. In TakeDeep, file organization replaces GUI configuration:

| Windsurf GUI Mode | TakeDeep Equivalent |
|---|---|
...
All three tools support the `AGENTS.md` standard. If you have one, it works in TakeDeep automatically:

```bash
# Verify it exists
ls -la AGENTS.md

# That's it - TakeDeep loads it automatically (enabled by default)
```

**Important:** Use uppercase `AGENTS.md` (not `agents.md`). TakeDeep also accepts `AGENT.md` (singular) as a fallback.

**Note:** Both `AGENTS.md` and `AGENT.md` are write-protected files in TakeDeep and require user approval to modify.

## Understanding Mode-Specific Rules

This is TakeDeep's unique feature that replaces both Cursor's `globs` and Windsurf's activation modes.
...
- [ ] **Verify rules loaded:** Click law icon (⚖️) in TakeDeep panel
- [ ] **Test rule application:** Ask TakeDeep to perform tasks following your rules
...
- **After migrating:** Organize files accordingly in TakeDeep

### Nested Rules Flattened

Cursor's nested directories don't map to TakeDeep. Flatten with descriptive names:
...
- **Check setting:** Verify "Use Agent Rules" is enabled in TakeDeep settings (enabled by default)
- **Reload:** Restart VS Code if needed

### Choosing Your Workflow

TakeDeep supports **both autocomplete and chat-driven workflows**. Choose the approach that fits your coding style, or combine them:
...
- [Join our Discord](https://deeper-api.takedeep.ai/discord) for migration support
