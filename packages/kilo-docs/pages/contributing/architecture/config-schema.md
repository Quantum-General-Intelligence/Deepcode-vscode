...
description: "How the Kilo CLI config JSON Schema is served at deeper-dash.takedeep.ai/config.json"
---

# CLI Config Schema

The JSON Schema referenced by `"$schema": "https://deeper-dash.takedeep.ai/config.json"` in `takedeep.json` files is served by the cloud repo. It is a runtime overlay of the upstream opencode schema with Kilo-specific additions on top.

## Flow

1. Client fetches `https://deeper-dash.takedeep.ai/config.json`.
...
2. Generate the JSON Schema shape: `bun --bun packages/opencode/script/schema.ts /tmp/takedeep.json`, then `jq '.properties.<new_key>' /tmp/takedeep.json`.
3. Paste the shape into the correct bucket in `apps/web/src/app/config.json/extras.ts` in the [cloud repo](https://github.com/Quantum-General-Intelligence/cloud).
...
If step 3 is skipped, users with `$schema: https://deeper-dash.takedeep.ai/config.json` will see "unknown property" warnings for the new key.
