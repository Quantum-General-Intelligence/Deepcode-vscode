---
title: "FAQ"
description: "Frequently asked questions about TakeDeep Claw"
---
...
- **From the dashboard** — Go to [https://deeper-dash.takedeep.ai/claw](https://deeper-dash.takedeep.ai/claw), select the model you want, and click **Save**. No redeploy is needed.

## Can I access the filesystem?

You can access instance files in `/root/.openclaw/` directly from the [TakeDeep Claw Dashboard](https://deeper-dash.takedeep.ai/claw). This is useful for examining or restoring config files. You can also interact with files through your OpenClaw agent using its built-in file tools.

## Can I access my TakeDeep Claw via SSH?

For security reasons, SSH access is currently disabled for all TakeDeep Claw instances. Our primary goal is to provide a secure environment for all users, and restricting direct SSH access is one of the many measures we take to ensure the platform remains safe and protected for everyone.

## How can I update my OpenClaw?

Do **not** click **Update Now** inside the OpenClaw Control UI — this is not supported for TakeDeep Claw instances and may break your setup.

Updates are managed by the TakeDeep Claw platform team to ensure stability. When a new version is available, it will be announced in the **Changelog** on your dashboard. To apply the update, click **Upgrade & Redeploy** from the [TakeDeep Claw Dashboard](/docs/kiloclaw/dashboard#redeploy).

## How do I migrate my OpenClaw?

Whether you're migrating from another OpenClaw provider to TakeDeep Claw, moving between TakeDeep Claw instances (e.g., individual to org or vice versa), or leaving TakeDeep Claw for another OpenClaw provider, you should plan to migrate your workspace, memory, and context so your new Claw retains the same knowledge as before.
...
If you are on TakeDeep Claw, you can use
