...
The `search_files` tool performs regex searches across multiple files in your project. It helps TakeDeep locate specific code patterns, text, or other content throughout your codebase with contextual results.
...
- When TakeDeep needs to find where specific functions or variables are used
- When TakeDeep helps with refactoring and needs to understand usage patterns
- When TakeDeep needs to locate all instances of a particular code pattern
- When TakeDeep searches for text across multiple files with filtering capabilities
...
- When asked to refactor a function, TakeDeep first searches for all places the function is used to ensure comprehensive changes.
- When investigating bugs, TakeDeep searches for similar patterns to identify related issues across the codebase.
- When addressing technical debt, TakeDeep locates all TODO comments across the project.
- When analyzing dependencies, TakeDeep finds all imports of a particular module.
