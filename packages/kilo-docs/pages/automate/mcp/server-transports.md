...
Model Context Protocol (MCP) supports two primary transport mechanisms for communication between TakeDeep and MCP servers: Standard Input/Output (STDIO) and Server-Sent Events (SSE). Each has distinct characteristics, advantages, and use cases.
...
1. The client (TakeDeep) spawns an MCP server as a child process
...
- **Locality**: Runs on the same machine as TakeDeep
...
- Single-client scenarios (one TakeDeep instance per server)
...
1. The client (TakeDeep) connects to the server's SSE endpoint via HTTP GET request
...
- **Remote Access**: Can be hosted on a different machine from TakeDeep
...
STDIO servers run locally on the same machine as TakeDeep, which has several important implications:
...
- **Execution**: Starts and stops with TakeDeep (child process lifecycle)
...
- Start when needed by TakeDeep
- Not require network configuration
- Need to be installed alongside TakeDeep or via a package manager
...
## Configuring Transports in TakeDeep

For detailed information on configuring STDIO and SSE transports in TakeDeep, including example configurations, see the [Understanding Transport Types](using-in-kilo-code#understanding-transport-types) section in the Using MCP in TakeDeep guide.
