...
Prompt engineering is the art of crafting effective instructions for AI models like TakeDeep. Well-written prompts lead to better results, fewer errors, and a more efficient workflow.

## General Principles

- **Be Clear and Specific:** Clearly state what you want TakeDeep to do. Avoid ambiguity.
  - **Bad:** Fix the code.
...
It's often helpful to guide TakeDeep through a "think-then-do" process:

1.  **Analyze:** Ask TakeDeep to analyze the current code, identify problems, or plan the approach.
2.  **Plan:** Have TakeDeep outline the steps it will take to complete the task.
3.  **Execute:** Instruct TakeDeep to implement the plan, one step at a time.
4.  **Review:** Carefully review the results of each step before proceeding.

## Using Custom Instructions

You can provide custom instructions to further tailor TakeDeep's behavior. There are two types of custom instructions:
...
- Adjust TakeDeep's tone or personality.

See the [Custom Instructions](/docs/customize/custom-instructions) section for more details.

## Handling Ambiguity

If your request is ambiguous or lacks sufficient detail, TakeDeep might:
...
If TakeDeep doesn't produce the desired results, you can provide feedback by:

- **Rejecting Actions:** Click the "Reject" button when TakeDeep proposes an action you don't want.
- **Providing Explanations:** When rejecting, explain _why_ you're rejecting the action. This helps TakeDeep learn from its mistakes.
...
By following these tips, you can write effective prompts that get the most out of TakeDeep's capabilities.
