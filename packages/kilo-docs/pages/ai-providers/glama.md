...
TakeDeep will automatically try to fetch a list of available models from the Glama API. Some models that are commonly available through Glama include:

- **Anthropic Claude models:** (e.g., `anthropic/claude-3-5-sonnet`) These are generally recommended for best performance with TakeDeep.
...
Glama is not yet available as a CLI provider. Check the [TakeDeep releases](https://github.com/Quantum-General-Intelligence/Deepcode-vscode/releases) for updates on provider support.
{% /callout %}
