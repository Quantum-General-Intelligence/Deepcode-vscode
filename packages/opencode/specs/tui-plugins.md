...
- Author package entrypoint is `@takedeep/plugin/tui`.
...
- Import types from `@takedeep/plugin/tui`.
- `@takedeep/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.
...
When `plugin` entries exist in a writable `.opencode` dir or `KILO_CONFIG_DIR`, OpenCode installs `@takedeep/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@takedeep/plugin/tui`.
