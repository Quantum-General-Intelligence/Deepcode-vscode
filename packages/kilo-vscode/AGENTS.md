...
| `packages/kilo-gateway/` | `@takedeep/gateway` | Auth (device flow), AI provider routing (OpenRouter), Kilo API integration (profile, balance, teams) |
| `packages/kilo-ui/` | `@takedeep/ui` | SolidJS component library (40+ components, built on `@kobalte/core`). Shared by this extension's webview and `packages/app/` |
| `packages/kilo-telemetry/` | `@takedeep/telemetry` | PostHog analytics + OpenTelemetry tracing for the CLI |
| `packages/kilo-i18n/` | `@takedeep/i18n` | Translation strings (16 languages) |
| `packages/kilo-docs/` | `@takedeep/docs` | Documentation site (Next.js + Markdoc) |

### Upstream OpenCode Packages (not Kilo-specific)

| Package | Name | Role |
|---|---|---|
| `packages/opencode/` | `@takedeep/cli` | Core CLI — forked from upstream OpenCode. AI agents, tools, sessions, server. |
...
New webview features must use **`@takedeep/ui`** components instead of raw HTML elements with inline styles. This is a Solid.js component library built on `@kobalte/core`.

- Import via deep subpaths: `import { Button } from "@takedeep/ui/button"`
...
- Global styles imported via `import "@takedeep/ui/styles"` in [`index.tsx`](webview-ui/src/index.tsx:2)
