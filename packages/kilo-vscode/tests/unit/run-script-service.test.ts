...
describe("RunScriptService", () => {
  ...
  it("builds commands for shell, PowerShell, and CMD scripts", () => {
    ...
    expect(buildRunTaskCommand({ path: "C:\\repo\\.takedeep\\run-script.ps1", kind: "powershell" })).toEqual({
      command: "powershell.exe",
      args: ["-NoLogo", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File", "C:\\repo\\.takedeep\\run-script.ps1"],
    })
    expect(buildRunTaskCommand({ path: "C:\\repo path\\.takedeep\\run-script.cmd", kind: "cmd" })).toEqual({
      command: "cmd.exe",
      args: ["/d", "/s", "/c", '"C:\\repo path\\.takedeep\\run-script.cmd"'],
    })
  })
  ...
  it.skipIf(process.platform === "win32")("rejects symlinks pointing outside the .takedeep directory", () => {
  ...
  it.skipIf(process.platform === "win32")("accepts symlinks pointing inside the .takedeep directory", () => {
