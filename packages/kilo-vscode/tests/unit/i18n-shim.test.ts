...
describe("t()", () => {
  it("returns translated string for known key", () => {
    const result = t("takedeep:autocomplete.statusBar.enabled")
    expect(typeof result).toBe("string")
    expect(result.length).toBeGreaterThan(0)
    expect(result).not.toBe("takedeep:autocomplete.statusBar.enabled")
  })
  ...
  it("interpolates a single variable", () => {
    const result = t("takedeep:autocomplete.statusBar.tooltip.noUsableProvider", {
      providers: "OpenAI, Anthropic",
      command: "command:takedeep.settingsButtonClicked",
    })
  ...
  it("interpolates multiple variables", () => {
    const result = t("takedeep:autocomplete.statusBar.tooltip.completionSummary", {
      count: "5",
      startTime: "10:00",
      endTime: "11:00",
      cost: "$0.05",
  ...
  it("leaves unreferenced vars intact in template", () => {
    const key = "takedeep:autocomplete.statusBar.tooltip.noUsableProvider"
